...
<div class="reveal" style="height: 500px; border: 1px solid #ccc;">
  <div class="slides">
    <section data-markdown="../zSLIDES/05-tdd.txt"
             data-separator="^\r?\n---\r?\n$"
...
```python title="test_validador_senha.py"
import pytest
...
```python title="validador_senha.py"
...
```python title="test_validador_senha.py" hl_lines="7-8"
import pytest
from validador_senha import validar_senha
...
```python title="validador_senha.py" hl_lines="5-11"
def validar_senha(senha):
    if len(senha) < 8:
        return False
...
```python title="test_validador_senha.py" hl_lines="10-14"
import pytest
from validador_senha import validar_senha
...
```python title="validador_senha.py" hl_lines="8-9"
def validar_senha(senha):
    if len(senha) < 8:
        return False
